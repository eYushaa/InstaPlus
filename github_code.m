...
static void traverseViewHierarchy(UIView *view, UIImageView * __strong *largestImageView, CGFloat *maxArea) {
    ...
    for (UIView *subview in view.subviews) {
        traverseViewHierarchy(subview, largestImageView, maxArea);
    }
}
...
static NSURL *extractURLFromPlayerLayer(CALayer *layer, NSMutableString *log) {
    ...
    // 2. FNFPlayerLayer veya diğer Instagram Video Layer'ları KVC taraması
...
static NSURL *extractVideoURLFromObjectInternal(id obj, int depth, NSMutableSet *visitedObjects, NSMutableString *log) {
    ...
    // 0. ÖNCELİK: IGVideo nesnesi üzerindeki allVideoURLs ve sortedVideoURLsBySize (Instagram v398+)
    @try {
            ...
            for (id u in (NSSet *)urlsObj) {
                if ([u isKindOfClass:[NSURL class]]) {
                    ...
                    if (![str containsString:@"oil_"] && ![str containsString:@".m3u8"]) {
                        [log appendFormat:@"[IGVideo] Found URL in allVideoURLs: %@\n", [(NSURL *)u lastPathComponent]];
                        return (NSURL *)u;
                    }
                }
            }
        ...
        if ([sortedObj isKindOfClass:[NSArray class]] && [(NSArray *)sortedObj count] > 0) {
            ...
            if ([first isKindOfClass:[NSDictionary class]]) {
                ...
                if ([u isKindOfClass:[NSString class]] && [(NSString *)u hasPrefix:@"http"]) {
                    [log appendFormat:@"[IGVideo] Found URL in sortedVideoURLsBySize\n"];
    ...
    // 1. ÖNCELİK: videoVersions / video_versions / typed_video_urls dizisindeki Progressive MP4 URL'leri
    ...
    for (NSString *propName in versionPropNames) {
        @try {
            ...
            if ([versions isKindOfClass:[NSArray class]] && [(NSArray *)versions count] > 0) {
                ...
                if (bestURL) {
                    [log appendFormat:@"[ProgressiveMP4] Found Progressive MP4 (%ldx%ld) in '%@' of %@\n", (long)maxArea, (long)maxArea, propName, NSStringFromClass([obj class])];
                    return bestURL;
                }
            }
        } @catch(NSException *e) {}
    ...
    // 2. ÖNCELİK: Alt modelleri tara (video, media, currentClipsItem, currentMedia, feedItem, item vb.)
    NSArray *subObjNames = @[
    ...
    // 3. FALLBACK: Doğrudan URL propları (Eğer videoVersions yoksa)
        ...
        @try {
            ...
            if ([val isKindOfClass:[NSURL class]]) {
                ...
                if ([str hasPrefix:@"http"] && ![str containsString:@"oil_"]) {
                    [log appendFormat:@"[ModelExtract] Fallback URL in '%@' of class %@\n", propName, NSStringFromClass([obj class])];
                    return (NSURL *)val;
                }
                ...
                if ([str hasPrefix:@"http"] && ([str containsString:@".mp4"] || [str containsString:@"/v/t"] || [str containsString:@"cdninstagram"]) && ![str containsString:@"oil_"]) {
                    [log appendFormat:@"[ModelExtract] Fallback String URL in '%@' of class %@\n", propName, NSStringFromClass([obj class])];
...
static void searchVisibleCellsForVideo(UIView *view, NSURL * __strong *foundURL, CGFloat *minDist, NSMutableString *log) {
    if (!view) return;
    ...
    if ([view isKindOfClass:[UICollectionView class]]) {
        ...
        for (UICollectionViewCell *cell in cv.visibleCells) {
            ...
            if (!CGRectIsNull(intersect) && intersect.size.height > 100) {
                ...
                if (dist < *minDist) {
                    NSURL *url = extractVideoURLFromObject(cell, log);
                    if (url) {
                        *minDist = dist;
                        *foundURL = url;
                        [log appendFormat:@"[VisibleCell] Found URL in centered CollectionViewCell (dist: %.0f)\n", dist];
                    }
                }
            }
        }
            ...
            if (!CGRectIsNull(intersect) && intersect.size.height > 100) {
                ...
                if (dist < *minDist) {
                    ...
                    if (url) {
                        *minDist = dist;
                        *foundURL = url;
                        [log appendFormat:@"[VisibleCell] Found URL in centered TableViewCell (dist: %.0f)\n", dist];
                    }
                }
            }
        }
    ...
    for (UIView *sub in view.subviews) {
        searchVisibleCellsForVideo(sub, foundURL, minDist, log);
    }
}

+ (NSURL *)extractActiveVideoURLFromScreenWithLog:(NSMutableString *)log {
    [log appendFormat:@"[MediaExtractor] Starting Active Video Extraction...\n"];
    ...
    // 1. ADIM: Oynatıcı Hook'undan Yakalanan Medya Nesnesi (videoVersions Progressive MP4)
    if (gLastPlayingMediaObject && (now - gLastPlayingVideoTime < 120.0)) {
        ...
        if (progURL) {
            [log appendFormat:@"[Success] Extracted Progressive MP4 from last playing media object! (age: %.1fs): %@\n", (now - gLastPlayingVideoTime), progURL.lastPathComponent];
            return progURL;
        }
    }
    
    // 2. ADIM: En Üstteki View Controller (TopViewController) Taraması
    UIViewController *topVC = getTopViewController(nil);
    if (topVC) {
        [log appendFormat:@"Top VC: %@\n", NSStringFromClass([topVC class])];
        NSURL *modelURL = extractVideoURLFromObject(topVC, log);
        if (modelURL) {
            [log appendFormat:@"[Success] Extracted Progressive MP4 from Top VC model!\n"];
            return modelURL;
        }
    }
    
    // 3. ADIM: Ekranda O An Merkezde Görünür Olan Cell Taraması (Reels / Feed / Story / DM)
    if (keyWindow) {
        NSURL *cellURL = nil;
        CGFloat minDist = CGFLOAT_MAX;
        searchVisibleCellsForVideo(keyWindow, &cellURL, &minDist, log);
        if (cellURL) {
            [log appendFormat:@"[Success] Extracted Progressive MP4 from centered media cell! Dist: %.0f\n", minDist];
            return cellURL;
        }
    }
    
    // 4. ADIM: Oynatıcı Hook'undan Yakalanan Canlı HTTP URL
    if (gLastPlayingVideoURL && (now - gLastPlayingVideoTime < 120.0)) {
        if (![gLastPlayingVideoURL isFileURL] && ![gLastPlayingVideoURL.absoluteString containsString:@"oil_"]) {
            [log appendFormat:@"[Success] Using active player hook HTTP URL (age: %.1fs): %@\n", (now - gLastPlayingVideoTime), gLastPlayingVideoURL.lastPathComponent];
            return gLastPlayingVideoURL;
        }
    }
    
    // 5. ADIM: Ekrandaki Görünür AVPlayerLayer / FNFPlayerLayer Taraması
    if (keyWindow) {
        NSURL *playerURL = nil;
        CGFloat minDist = CGFLOAT_MAX;
        ...
        if (playerURL && ![playerURL isFileURL] && ![playerURL.absoluteString containsString:@"oil_"]) {
            [log appendFormat:@"[Success] Extracted URL from active player layer! Dist: %.0f\n", minDist];
            return playerURL;
        }
    }
    
    // 6. ADIM: Disk Önbelleği Taraması (Sadece Gerçek .mp4 Dosyaları, oil_ Parçalı Dosyaları ATLA)
    [log appendFormat:@"Scanning disk cache for full .mp4 video files (skipping chunked oil_ files)...\n"];
    NSArray *searchPaths = @[
    ...
    for (NSString *basePath in searchPaths) {
        ...
        for (NSURL *fileURL in enumerator) {
            ...
            // 2 saniyelik parçalı oil_ ve stream_ tampon dosyalarını ATLA
            if ([fileName hasPrefix:@"oil_"] || [fileName containsString:@"stream_"] || [fileName hasPrefix:@"chunk_"]) {
                continue;
            }
            ...
            if ([fileName hasSuffix:@".mp4"]) {
                ...
                if (age < 30.0) {
                    ...
                    // 2-3 saniyelik parçalı dosyalar genelde < 500KB olur. Tam video en az 1.5MB'tır.
                    if (fileSize.longLongValue > 1500000) { 
                        validFiles++;
                        [log appendFormat:@"[DiskCache] Found full video: %@ (%.1f MB, Age: %.1fs)\n", fileURL.lastPathComponent, fileSize.doubleValue / (1024.0 * 1024.0), age];
    ...
    [log appendFormat:@"Scanned:%d, Full Disk Cache Videos Found:%d\n", scannedFiles, validFiles];
    if (newestVideoURL) {
        [log appendFormat:@"[Success] Selected Disk Cache Full MP4: %@ (Age: %.1fs)\n", newestVideoURL.lastPathComponent, freshestAge];
        return newestVideoURL;
    }
    
    [log appendFormat:@"All video extraction methods failed.\n"];
    return nil;
}
